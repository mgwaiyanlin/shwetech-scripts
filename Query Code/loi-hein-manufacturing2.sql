WITH RequiredItems AS (
    ...
    GROUP BY wo.name, wo.production_item, woi.item_code
),
...
ConsumedItems AS (
    ...
    GROUP BY se.name, se.work_order, sei.item_code
),
...
FinishedItems AS (
    SELECT
        ...
        MAX(sei.basic_rate) AS basic_rate,
        se.total_additional_costs AS add_cost,
        (MAX(sei.basic_rate) + (se.total_additional_costs / NULLIF(SUM(sei.qty),0))) AS actual_cost_rate,
        se.posting_date
    FROM `tabStock Entry` se
    INNER JOIN `tabStock Entry Detail` sei ON sei.parent = se.name
    WHERE
    ...
    GROUP BY
        se.name, se.work_order, sei.item_code, se.posting_date, se.total_additional_costs
),
...
ScrapItems AS (
    ...
    WHERE
        ...
        AND se.posting_date BETWEEN %(from_date)s AND %(to_date)s
    GROUP BY se.name, se.work_order, sei.item_code, se.posting_date
),
...
SELECT
    ...
    mi.actual_cost_rate AS "Input Cost Rate:Currency:150",
    mi.basic_rate AS "System Cost Rate:Currency:150",
    mi.add_cost AS "Additional Cost:Currency:150",

    (mi.actual_cost_rate * fi.produced_qty) AS "Total Input Amount:Currency:150",
    (mi.basic_rate * fi.produced_qty) AS "Total System Amount:Currency:150",
...
ORDER BY
    ...
    FIELD(mi.status,
        'Matched with BOM', 'Extra Used (Not in BOM)', 'Missing Usage (Not Consumed)', 'Scrap Item'
    ),
    mi.raw_item_code;
